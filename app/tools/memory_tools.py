...
*美食之旅(吃货、美食、小吃)
    ...
    if not user_id:
        return '未识别到用户身份，无法保存旅行偏好'
    try:
        service = await get_user_memory_service()
        await service.update_travel_style(user_id,styles)
...
avg_budget_per_night：平均每晚预算(元），可选
    ...
    :param avg_budget_per_night:
    :param runtime:
    :return:
    """
    user_id =runtime.state.get('user_id')
    if not user_id:
        return '未识别到用户身份，无法保存住宿偏好'
    if not preferred_types and not avg_budget_per_night:
        return '请至少提供住宿类型或预算信息'
    try:
        service = await get_user_memory_service()
        ...
        if avg_budget_per_night:
            result_parts.append(f'预算-{''.join(avg_budget_per_night)}/晚')
...
提供更个性化的推荐
    ...
    try:
        ...
        app_logger.error(f'保存出行')
        return f'保存失败'
#工具导出
MEMORY_TOOLS =[
    update_accommodation_preference_tool,##保存住宿偏好
    update_travel_style_tool,##保存旅行风格
    update_dietary_restriction_tool,##保存饮食禁忌
    update_food_preference_tool,##保存饮食偏好
    add_travel_record_tool,##保存出行历史
]
